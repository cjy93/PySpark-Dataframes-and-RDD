...
fn_longlat = "C:/Users/jia yi/Desktop/NSDBA2/IT8703 Streaming analytics/ca1/Dataframe CA sol/createdData/long_lat_district_projects_All.csv"
...
layout = html.Div(children=[
    html.H1(children='Does District play a role in attractiveness?',style ={'text-align': 'center'}),  
    html.Div(children=[
    ...
    #'''),
    # map
    dcc.Graph(id='All_map', style= {'width': '79%', 'display': 'inline-block'}), 
...
@app.callback(
    dash.dependencies.Output('districtName', 'options'),
...
def update_availableDistrict(selectedFilter):    
    ...
    for row in dropdownlist.iterrows():
        district = row[1][0]
        districtName = row[1][2]
        if int(district) in list_filtered_district:
            print('True: {}'.format(district))
            dict_district = {'label': districtName, 'value': district}
...
@app.callback(
    dash.dependencies.Output('Project', 'value'),
    [   dash.dependencies.Input('districtName', 'value'),
        dash.dependencies.Input('topUnsold', 'value'),
